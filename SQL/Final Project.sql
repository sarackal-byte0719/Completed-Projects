...
#Question 1 10 pts
#Given a table employee, write a query to return alternative rows.
...
#Question 210 pts
#Write a query to return the highest 5 salaried employees.
SELECT *
FROM employees a
...
#Question 310 pts
#Write a query to return the top 2 salaried employees in each department.
...
#Question 410 pts
#Write a query to return the top 2 salaried employees in each department and average department salary.
SELECT distinct salary, employee_id,  DEPARTMENT_id, avg(SALARY) as AVGSalary
FROM employees a
WHERE 2 > (SELECT COUNT(DISTINCT salary)
FROM employees b 
WHERE b.salary >= a.salary AND a.Department_ID = b.Department_ID) 
...
FROM employees a where (select avg(a.salary)  FROM employees b 
WHERE b.salary >= a.salary AND a.Department_ID = b.Department_ID) 
ORDER BY a.Department_ID desc;

select * from emp e, (select dept,avg(sal) avg_sal from emp group by
dept) e1
where e.dept=e1.dept and e.sal > e1.avg_sal
